...
class UserService(object):
    ...
    def register_new_user(request_json):
        ...
        if not facebook_token_is_valid:
            return ApplicationResponse.bad_request_message("Invalid Facebook credentials")
        ...
        if shared_server_response_validation.hasErrors:
            return ApplicationResponse.bad_request_message(shared_server_response_validation.message)
        UserRepository.insert(request_json)
        return ApplicationResponse.created_message("Created user successfully")
    ...
    def authenticate_user(request_json):
        ...
        return ApplicationResponse.success(response_json)
    ...
    def send_user_friendship_request(request_json):
        ...
        if UserRepository.username_exists(target_username):
            ...
            return ApplicationResponse.success_message("Friendship request sent successfully")
        return ApplicationResponse.bad_request_message("Target username doesn't exist")
    ...
    def get_friendship_requests(request_header):
        ...
        return ApplicationResponse.success(friendship_list)
    ...
    def accept_friendship_request(request_header, target_user):
        ...
        if FriendshipRepository.friendship_exists(user_that_accepts_friendship, target_user):
            ...
            return ApplicationResponse.success_message("Friendship was accepted successfully")
        return ApplicationResponse.bad_request_message("Friendship request couldn't be found")
    ...
    def create_user_profile(request_json, request_header):
        ...
        return ApplicationResponse.created_message("Created profile successfully")

    @staticmethod
    def get_user_profile(username):
        profile = UserRepository.get_profile(username)

        return ApplicationResponse.success(dumps(profile))
